...
public class AppointmentTools {
    ...
    @Tool(name = "预约挂号",
            value = "根据参数，先执行工具方法queryDepartment查询是否可预约，并直接给用户回答是否可预约，并让用户确认所有预约信息，用户确认后再进行预约。")
    public String bookAppointment(Appointment appointment) {
        //查找数据库中是否包含对应的预约记录
        Appointment appointmentInDB = appointmentService.getOne(appointment);
    ...
    public String cancelAppointment(Appointment appointment) {
        ...
        if (appointmentInDB != null) {
            //删除预约记录
            if (appointmentService.removeById(appointmentInDB.getId())) {
                return "取消预约成功";
            } else {
                return "取消预约失败";
            }
